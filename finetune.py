...
if args.test:
    ...
    for i, ts_data in enumerate(testloader):
        ...
        loss = torch.sqrt(criterion(outputs3d[0, -1], labels3d))
        error.append(loss.item())
        loss_2d = torch.sqrt(criterion(outputs2d[0, -1], labels2d))
        error_2d.append(loss_2d.item())
    print('test error (3d): %.5f ± %.5f, f@10: %.5f, f@20: %.5f' % (np.mean(error), np.std(error), calculate_f_score(error, 10), calculate_f_score(error, 20)))
    np.save('finetune3d.npy', error)
    print('test error (2d): %.5f ± %.5f' % (np.mean(error_2d), np.std(error_2d)))
    np.save('finetune2d.npy', error_2d)