...
if __name__ == '__main__':
    ...
    if args.pretrained_model != '':
        state_dict = torch.load(args.pretrained_model, map_location=torch.device('cpu'))  # 加载模型状态字典
        new_state_dict = {}
        for key, value in state_dict.items():
            if key.startswith('module.'):  # 检查键是否以'module.'开头
    ...
    if args.test:
        ...
        for i, ts_data in enumerate(testloader):
            ...
            outputs3d = model(labels2d)
            loss =  criterion(outputs3d, labels3d)
            running_loss += np.sqrt(loss.data)
